...
const investmentOptions: ButtonOption[] = [
  { value: 'investment-fundos', label: 'Fundos', color: '#2567F9' },
  { value: 'investment-tesouro-direto', label: 'Tesouro', color: '#8F3CFF' },
  { value: 'investment-previdencia', label: 'Previdência', color: '#FF3C82' },
  { value: 'investment-bolsa', label: 'Bolsa', color: '#F1823D' },
];

export const DashboardServices = ({ userName = 'Usuário' }: DashboardServicesProps) => {
  ...
  const servicesWithActions: Service[] = [
    ...
    { id: '2', title: 'Meus cartões', description: '', icon: '/DashboardServices/cartao.svg', action: () => setActiveSection('cards') },
    { id: '3', title: 'Doações', description: '', icon: '/DashboardServices/doacoes.svg' },
    { id: '4', title: 'Pix', description: '', icon: '/DashboardServices/pix.svg' },
    { id: '5', title: 'Seguros', description: '', icon: '/DashboardServices/seguros.svg' },
    { id: '6', title: 'Crédito celular', description: '', icon: '/DashboardServices/celular.svg' },
  ];
  ...
  const rendaFixa = transactions
    .filter(
      (t) =>
        t.type === 'investment' &&
        (t.investmentType === 'tesouro-direto' || t.investmentType === 'previdencia' || t.subtype === 'renda-fixa')
    )
    .reduce((total, t) => total + t.amount, 0);
  ...
  const rendaVariavel = transactions
    .filter(
      (t) =>
        t.type === 'investment' &&
        (t.investmentType === 'fundos' || t.investmentType === 'bolsa' || t.subtype === 'renda-variavel')
    )
    .reduce((total, t) => total + t.amount, 0);
              ...
              <div className={styles['transaction-form']}>
                <Dropdown
                  ...
                  onChange={(value) => {
                    ...
                    if (value !== 'investment') setSelectedInvestmentType('');
                  }}
                />
        ...
        return (
          <>
            ...
            <div className={styles['investment-total']}>
              ...
              <p className={styles['total-value']}>{formatCurrency(totalInvestments)}</p>
            </div>
            ...
            <div className={styles['investment-categories']}>
              <div className={styles['investment-card']}>
                ...
                <p className={styles['card-value']}>{formatCurrency(rendaFixa)}</p>
              </div>
              <div className={styles['investment-card']}>
                <h3 className={styles['card-title']}>Renda Variável</h3>
                <p className={styles['card-value']}>{formatCurrency(rendaVariavel)}</p>
              </div>
            </div>
        ...
        return (
          <>
            ...
            <div className={styles['empty-state']}>
              <p className={styles['empty-message']}>Outros serviços em desenvolvimento...</p>
  ...
  return <section className={styles.services}>{renderContent()}</section>;
};
